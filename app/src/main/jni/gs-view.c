...
//            "! rtpjitterbuffer drop-on-latency=true max-dropout-time=150 "
//            "! rtph264depay "
//            "! h264parse "
//            "! queue ! decodebin "
//            "! queue ! videoconvert"
//            "! glimagesink async=false enable-last-sample=false qos=false sync=false",
//            &error);

  data->pipeline = gst_parse_launch ("appsrc name=source is-live=true min-latency=80 max-latency=150 ! h264parse ! decodebin name=decoder decoder. ! glimagesink async=false enable-last-sample=false qos=false sync=false",&error);

  //data->pipeline = gst_parse_launch ("appsrc name=source is-live=true max-latency=100 ! h264parse ! amcviddec-omxgoogleh264decoder ! glimagesink async=false enable-last-sample=false qos=false sync=false max-lateness=150",&error);
...
gst_native_init (JNIEnv * env, jobject thiz)
{
  ...
  GST_DEBUG_CATEGORY_INIT (debug_category, "gs-view", 0,
      "Android tutorial 3");
  gst_debug_set_threshold_for_name ("gs-view", GST_LEVEL_DEBUG);
...
static jboolean
gst_native_class_init (JNIEnv * env, jobject thiz)
{
  ...
  if (!custom_data_field_id || !set_message_method_id
     ...
     */
    __android_log_print (ANDROID_LOG_ERROR, "gs-view",
...
static JNINativeMethod native_methods[] = {
  ...
  {"nativeReceiveVideoData", "([B)V", (void *) gst_native_receive_video_data}
...
JNI_OnLoad (JavaVM * vm, void *reserved)
{
  ...
  jclass klass = (*env)->FindClass (env,
                                    "com/kolaa/djifpvviewer/MainActivity");
